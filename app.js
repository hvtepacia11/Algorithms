...
async function apiPost(path, body) {
  try {
    ...
    // Only show alert for network errors, not for failed HTTP responses
    if (err.message.includes('Failed to fetch') || err.message.includes('NetworkError')) {
...
// =================== GREEDY ALGORITHMS INITIALIZATION ===================
...
// =================== MATHEMATICAL OPERATIONS WITH FULL STEPS ===================
...
// Update the math operations to handle floating-point numbers
async function runMathOperation() {
  const operation = $('mathOperation').value;
  const base = Number($('mathBase').value);
  const num1 = $('mathNum1').value.trim();
  ...
  // Validate floating-point numbers for the given base
...
// =================== BUTTER-SMOOTH ZOOM FUNCTIONALITY ===================
let huffmanZoom = 1.0;
let treeZoom = 1.0;
const MIN_ZOOM = 0.1;
const MAX_ZOOM = 5.0;
...
async function playOps(ops, algo){
  ...
  // Track gap for Shell Sort and Comb Sort
  let currentGap = null;
  let gapSequence = [];
  ...
  if (algo === 'tim') {
    ...
    // Divide into runs (simplified - 4 elements per run)
    const runs = [];
    ...
    // Show insertion sort for each run
    runs.forEach((run, index) => {
      addSortStep(`Sort Run ${index + 1} using insertion sort`);
      addSortStep(`[${run.join(', ')}]`);
      
      // Simulate insertion sort steps for this run
    ...
    if (algo === 'selection') {
      ...
      if (op.type === 'set') {
        ...
        if (el) {
          ...
          // Show final result only once when completely sorted
      ...
      // Shell and Comb sort (existing code)
...
// =================== TOURNAMENT/HEAP/TREE SORT WITH ZOOM ===================
...
async function runHeapSortVisual(arr){
  ...
  async function heapify(nsize, i){
    while (true){
      ...
      if (largest!==i){ 
        ...
        drawHeapTree(a, new Set([i,largest])); 
        await sleep(delay); 
        i=largest;
  ...
  for (let end=n-1; end>0; end--){ 
    ...
    // Only draw the current heap portion (0 to end-1)
    const currentHeap = a.slice(0, end);
    drawHeapTree(currentHeap, new Set([0, end])); 
    await sleep(delay); 
  ...
  // Handle the final 1-node case explicitly
...
if (moreStartBtn) moreStartBtn.addEventListener('click', async ()=>{
  ...
  try {
    ...
    // Handle server-side algorithms - CORRECTED VERSION
      ...
      if (algo === 'extended-euclidean') {
        ...
        endpoint = '/api/more';  // CHANGED: Use /api/more instead of /api/math/extended-euclidean
        requestBody = {
          algo: 'extended-euclidean',  // ADDED: Specify which algorithm
          a: parseInt(aRaw),
          b: parseInt(bRaw)
        };
      } 
        ...
        endpoint = '/api/more';  // CHANGED: Use /api/more instead of /api/math/chinese-remainder
        requestBody = {
          algo: 'chinese-remainder',  // ADDED: Specify which algorithm
          pairs: bRaw
        };
      }
...
function drawHuffmanTree(struct) {
  ...
  // Reset zoom and pan for Huffman tree
  huffmanZoom = 1.0;
  huffmanTranslateX = 0;
  huffmanTranslateY = 0;
  ...
  // Dynamic canvas sizing with better spacing
  const levelGap = 120;
  const nodeRadius = 28;
  const horizontalSpacing = 80;
  
  // Calculate required width based on leaf count and spacing
  ...
  function calculatePositions(node, depth, leftBound, rightBound) {
    ...
    // For leaf nodes, position them evenly
    ...
    // For internal nodes, recursively position children
    ...
    // Position current node between its children
  ...
  // Calculate all positions with proper bounds
  ...
  function drawEdges(node) {
    ...
    // Draw edge to left child with '0' label
    if (node.left) {
    ...
    // Draw edge to right child with '1' label
    if (node.right) {
...
// Add the missing zoom transform function
...
// Make sure the setupHuffmanZoom function is also available
function setupHuffmanZoom() {
  const canvas = document.getElementById('hufCanvas');
  const canvasContainer = document.querySelector('.canvas-container');
  const zoomLevel = document.getElementById('zoomLevel');
...
// Also update the renderHuffman function to ensure proper initialization
function renderHuffman(resp, freqMap) {
  ...
  // Setup zoom after a short delay to ensure DOM is updated
  setTimeout(() => {
    setupHuffmanZoom();
  }, 100);
}
...
async function runCryptoAlgorithm(operation) {
  ...
  try {
    ...
    console.log('Sending crypto request:', requestData);
